...
export default function PersonalLeaseListing({ cars }: CarRentalListingProps) {
  ...
  const latestYear = Math.max(
    ...filteredAndSortedCars.flatMap((car) =>
      car.personal_lease_pricing.pricing_matrix.map((p) => Number(p.year_title))
    )
  )
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        {filteredAndSortedCars.length > 0 ? (
          filteredAndSortedCars.map((car) => (
            <Card className="overflow-hidden border border-gray-200 transition-shadow hover:shadow-lg">
              {/* Image area (clickable) */}
              <Link
                href={`${pathname}/${car.slug}`}
                className="relative block aspect-[16/9] cursor-pointer bg-gray-100"
              >
                {car.personal_lease_pricing.pricing_matrix.some(
              ...
              <CardContent className="px-4 py-0">
                ...
                <div className="mb-4 flex items-center gap-4 text-sm text-gray-600">
                  <span>
                    {
                      car.personal_lease_pricing.pricing_matrix.reduce(
                        (latest, current) =>
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-bold text-red-500">
                      {Number.parseFloat(
                        car.personal_lease_pricing.pricing_matrix.reduce(
                          (latest, current) =>
                            Number(current.year_title) >
                            Number(latest.year_title)
                              ? current
                              : latest
